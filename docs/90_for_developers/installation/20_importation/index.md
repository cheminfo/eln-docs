...
`rest-on-couch` allows you to automatically import files based on a filter placed in the corresponding folder.
...
To debug a file that is in errored state and to edit the `import.js` file, you may run the import in an interactive way. For this you may go in the docker image. First find the `id` of the `rest-on-couch-import` image using `docker ps` and then go in the image using `docker exec -it fe2acfbb9bba sh` (replace the correct image ID).

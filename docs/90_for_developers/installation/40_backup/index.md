...
Since CouchDB is an append-only database, using a [rsync](https://rsync.samba.org/)/[rsnapshot](https://rsnapshot.org/)-based solution is a safe option. For large databases you may be interested in the `rsync` option `--append` that will only append data onto shorter files and therefore not copy the full database.
